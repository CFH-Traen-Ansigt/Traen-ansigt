...
export default function TaskCard({
  ...
  const [numberOfRepititions, setNumberOfRepititions] = useState(repititions || 5);
  ...
  return (
    <div className="flex gap-2" draggable={variant === "small"} onDragStart={onDragStart} onDragEnter={onDragEnter} onDragEnd={onDragEnd} onDragOver={onDragOver}>
      {variant === "small" && <img draggable={false} src="/assets/three-dots.svg" alt="dots icon" className={`${variant === "small" && "hover:cursor-pointer"}`} />}
      <article
        className={`${
          variant === "small" ? "w-[300px] h-[125px] px-2 py-2 cursor-pointer" : "w-[420px] max-h-[300px] px-4 py-4"
        } flex gap-2  rounded-md bg-main-color border-solid border-font-color border-1`}
      >
        <img draggable={false} className={`${variant === "small" ? "w-[107px] h-[107px] self-center" : "w-[153px] h-[153px]"} object-contain`} src={`${image}`} alt={`${title}`} />
          ...
          <div className="flex items-center mt-auto justify-between w-full">
            ...
            <Button
              ...
              onClick={() => {
                if (variant === "small") {
                  setTasks((old) => {
                    const updatedTasks = old.filter((item) => item.exerciseNo !== exerciseNo);
                    return [...updatedTasks];
                  });
                } else {
                  addTask();
              ...
              variant={variant === "small" ? "Cancel" : isSelected ? "Inverted" : "Primary"}
              icon={variant === "small" ? "Trashcan" : isSelected ? "InvertedPlusIcon" : "PlusIcon"}
              styling={` rounded-lg ${variant === "small" ? "px-2 gap-1 h-7 text-[16px] pt-[3px]" : "px-3 gap-2 h-8 text-[18px] pt-[3.5px]"}`}
              iconStyling={`w-[20px] h-[20px] ${variant === "small" && "mt-0 w-[16px]  h-[16px]"}`}
              disabled={isSelected}
            />
          </div>
        </div>

...
export default function ProgramCard({
  ...
  return (
    <article className={`relative w-[300px] min-h-[480px] ${addShadow && "border-dashed border-[2px] border-secondary rounded-lg  cursor-pointer"} p-3 flex flex-col`}>
      ...
      <div className={`w-[240px] mx-auto ${addShadow && "opacity-40"}`}>
        <img src={image} alt={`${title}`} className="w-full h-full object-cover" />
      </div>
      <div className={`flex flex-col flex-grow w-[240px] mx-auto ${addShadow && "opacity-40"}`}>
        {" "}
        ...
        <div className="mt-auto">
          <div className="flex justify-between w-full mt-4 mb-4">
            <div className="flex gap-2 items-center w-1/2 ">
              <img src="/assets/black-clock.svg" alt="clock icon" className="w-6" /> <p className="font-light text-lg pt-2">{duration} min.</p>
            </div>
            <div className="flex gap-2 items-center w-1/2">
              <img src="/assets/dumbbell.svg" alt="dumbbell icon" className="w-6" /> <p className="font-light text-lg pt-2">{totalExercises} Øvelser</p>
            </div>
          </div>
